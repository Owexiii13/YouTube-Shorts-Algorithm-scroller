...
**All code generated by Manus AI, Gemini 3 Pro, Claude Opus 4.5, Claude Opus 4.6, Gemini 3.1 Pro**

## Backend API (current + compatibility)

The backend keeps existing endpoints for compatibility and now supports a new anonymized logging flow.

### New endpoints

#### `POST /log_video`
Log a single anonymized watch/action event.
...
Notes:
- `watch_percentage` is validated to the range `0..1`.
- `user_action` must be one of:
  - `viewed`, `scrolled`, `liked`, `disliked`, `shared`, `commented`, `trusted_channel`, `blocked_channel`
- `video_id` and `channel_id` are optional in this new flow.
- Safe defaults:
  - `session_id`: `"anonymous"`
  - `watch_percentage`: `0.0`
  - `watch_duration_seconds`: `0.0`
  - `metadata`: `{}`

#### `GET /chunk_status`
Check chunk ingestion state.

Optional query string:
- `chunk_id` (optional): if provided, returns status for one chunk; otherwise returns aggregate status.

#### `POST /submit_chunk`
Submit one chunk of batched events.

Example payload:

```json
{
  "chunk_id": "batch-2026-01-20-user-1",
  "chunk_index": 0,
  "total_chunks": 3,
  "events": [
    {
      "session_id": "anon-session-1",
      "watch_percentage": 0.21,
      "user_action": "scrolled",
      "metadata": {
        "source": "extension"
      }
    }
...
- `chunk_index >= 0`
- `total_chunks >= 1`
- each event uses the same validation as `POST /log_video`
